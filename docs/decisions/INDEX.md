...
## [DR-066 — Audience-partitioned approvables — a product-owner-facing review split, derived from paragraph type](DR-066.md)

*Status: proposed · Date: 2026-07-21*

<!-- dr-summary:DR-066 auto=0000000000000000 -->
Specs interleave a product-owner audience (what/why — FR/AC/OQ/G/M) with an engineering audience (how — NFR/CT/RD/INV/CR/TV/FM), and today there is no audience/owner primitive anywhere; approval is per-spec-file (SPEC-022), single-approver-suffices, with no sub-file unit. This records a two-phase, universal (not opt-in; no corpus backfill) audience partition: Phase B rides the existing split-file boundary (requirements.md = PO approvable, design/tasks = engineering) using the per-file approval sidecars that already exist — zero new parsing, ships today; Phase C derives audience from the DR-053 paragraph-type code (blocked on DR-053 + #679), producing a filtered PO view and a paragraph-projection approval hash extending SPEC-022 FR-3. Audience is a property of content type, not a person. Derived beats declared (can't rot, no gate needed — RCDD/DR-003). The approval ingress is multi-surface and audience-agnostic — the partition governs routing, not who may approve. For Phase B the networked surface is GitHub itself, not a bespoke webview: CODEOWNERS routes requirements.md → PO team and design/tasks → engineering as required reviewers, anyone approves via GitHub's native review, and a consent-gated Action materialises the SPEC-022 sidecar attributing the human approver — the offline MinSpec: Approve path stays, so the invariant holds. This delivers mobile/web approval for devs and POs alike (a VS Code webview is desktop-bound and can't run on a phone). A bespoke SPEC-014-style webview is retained only as the Phase-C fallback (sub-file projection GitHub can't express). A corpus jargon audit (≈96% of PO-facing paragraphs carry developer jargon) makes an authoring-accessibility guideline + lint a precondition, not a nicety.
<!-- /dr-summary:DR-066 -->
