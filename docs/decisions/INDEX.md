...
## [DR-066 — Audience-partitioned approvables — a role-based review split by file separation, with a PO-only auto-approve mode](DR-066.md)

*Status: proposed · Date: 2026-07-21*

<!-- dr-summary:DR-066 auto=0000000000000000 -->
Specs interleave a product-owner audience (what/why) with an engineering audience (how), and today there is no audience/owner primitive; approval is per-spec-file (SPEC-022), single-approver-suffices, with no sub-file unit. This records a universal (not opt-in; no corpus backfill), role-based audience partition where audience = whole files mapped to roles. Four decisions: (1) keep each audience in separate files (requirements.md = PO, design/tasks = engineering) — the per-file approval sidecars already make each file its own hash-bound approval unit, so sub-file paragraph projection ("Phase C", deriving audience from DR-053 type codes) is DROPPED, not deferred, removing the DR-053/#679 dependency and matching the per-file granularity of GitHub/CODEOWNERS/sidecars; (2) the audience→file→role map is N-role extensible from the start, so future teams-version roles (UX designer, marketing copy/proofreader, BA/producer) slot in as more files + CODEOWNERS globs; (3) the approval ingress is multi-surface and audience-agnostic — offline MinSpec: Approve plus GitHub-native review that closes the mobile gap for ALL committers (a VS Code webview is desktop-bound), with a consent-gated Action materialising the sidecar attributing the human approver, so the offline invariant holds; PRs are NOT split by audience by default (approval is already file-scoped, so splitting buys only visual hiding at an atomicity cost — opt-in only); (4) a PO-only mode auto-approves technical approvables for solo devs upgrading from vibe-coding, via a per-audience config policy (human|auto) — recorded as a distinct "auto-approved by policy" record that does NOT launder DR-056's human-approver gate or make the never-wrong signpost lie, and that waives human review but NOT the deterministic machine gates (validate, SPEC-023, SPEC-024). A corpus jargon audit (≈96% of PO-facing paragraphs carry developer jargon) makes an authoring-accessibility guideline + lint a precondition, not a nicety.
<!-- /dr-summary:DR-066 -->
