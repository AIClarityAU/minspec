...
scan_file() {
  ...
  for pat in "${CS_PATTERNS[@]}"; do
    out=$(grep -nE -e "$pat" -- "$f" 2>/dev/null || true)  # swallow-known: #1978 grep exit 2 (unreadable file) reads as no finding, in a security scanner
    if [[ -n "$out" ]]; then printf '%s\n' "$out" | emit shape "$f"; found=1; fi
  done

  for pat in "${CI_PATTERNS[@]}"; do
    out=$(grep -niE -e "$pat" -- "$f" 2>/dev/null || true)  # swallow-known: #1978 grep exit 2 (unreadable file) reads as no finding, in a security scanner
