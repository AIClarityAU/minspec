...
function runWithStub(opts: { stdout?: string; stderr?: string; code?: number }): {
  ...
  fs.writeFileSync(
    ...
    `#!/usr/bin/env bash
# DR-077 preflight probes \`claude -p --help\` for --json-schema; advertise it.
...
/**
 * DR-077: the reviewer now RETURNS the verdict. The prose still discusses quota —
...
describe('#1131 review-branch.sh — genuine outages and crashes are unchanged', () => {
  ...
  it('a MALFORMED structured verdict is not treated as a review', () => {
    // Replaces the old truncated-block case: under DR-077 nothing parses prose, so
