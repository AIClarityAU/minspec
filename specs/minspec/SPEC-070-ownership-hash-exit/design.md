...
SPEC-070 was **APPROVED** when this section was written. **All six items below have since been accepted by the founder and applied**, batched onto `docs/spec-070-amendments` as PR #2101 - items 1-4 in commit `e6e76052`, items 5-6 in `69c32422`. They were batched deliberately, because each separate edit stales the approval and freezes this spec's 14 declared paths until it is re-approved, so six edits would have cost six re-approvals for one outcome. The consequence is the point rather than a side effect: `facts approval SPEC-070` now reports `STALE`, and ➡️ **the founder's re-approval is a precondition for every slice that writes a declared path.**
