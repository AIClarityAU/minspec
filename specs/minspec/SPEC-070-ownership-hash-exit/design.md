...
export interface ApprovalRecord {
  ...
  /** OQ-9, decided 2026-09-25 - record it, do not deny on it. The trimmed `implements_reason:` text at approve time, for the
...
**OQ-2 - what do adopters get? SPLIT. One half was the designer's call, one half went to the founder and was ACCEPTED on 2026-09-25, and one adopter-visible change had ALREADY shipped by the time the question was asked.**
...
- **(ii) The head verdict. ACCEPTED: build the third verdict, founder, 2026-09-25.** The recommendation was a third verdict - a record valid under the frozen pre-strip basis reports `PRE-STRIP BASIS - matches under the pre-ownership-strip hash; re-approve to move it forward`, distinct from MATCHES and never green - rather than `MISMATCH - a real finding`. **The cost, named because a recommendation without one is advocacy:** it needs a second basis inside a Python script, which is the same unbuildable requirement (i) just retreated from, so accepting (ii) means accepting a managed `canonical-pre-strip.py` template, a moved count pin and a fourth twin under INV-5; and it changes what MinSpec ships into every opted-in adopter repo, contradicting the last paragraph of **FR-11** (requirements.md:625-631) and **AC-23** (`:827-832`). Amending an approved spec's FR and AC is a human act, and reversibility in this repo does not reverse what already shipped into adopters. The founder accepted the recommendation and its stated cost, so the contradiction was resolved the only way it can be - by amending the two paragraphs rather than shipping against them. **FR-11's closing paragraph and AC-23 are amended** on `docs/spec-070-amendments` (PR #2101, amendments 5 and 6), which means `requirements.md` is now `STALE` and slice 8 cannot start until the founder re-approves the spec. **A first attempt at those two amendments was blocked by the review panel, unanimously and correctly**, for a defect this section's own framing invited: amending FR-11 and AC-23 to cite a resolved OQ-2 while leaving the OQ-2 **register** entry open, still listing (a)/(b)/(c) and recommending (b). The spec read as open and resolved at once. The fix records the resolution where the register lives, as a fourth option **(d)** - ship the widened hasher to everyone with no flag and no migration, plus a frozen, diagnosis-only pre-strip basis so the tool can report the third verdict - and records the decision itself in **DR-096** (born `proposed`), because SPEC-051 INV-2 requires a change to the DR-034 hashing contract to carry its own DR. *Neither the register resolution nor `docs/decisions/DR-096.md` exists in the tree this document is committed to; both live on the amendment branch. See the notice at the head of section 8.* One further trap the panel named: **"the third verdict" is not option (c)**. It means a third provenance *verdict string* beside `MATCHES` and `MISMATCH`; (c) is the opposite and accepts the `MISMATCH`. The three costs named above are now committed work, not hypotheticals: a managed `canonical-pre-strip.py` template, the membership-count pin move, and a fourth twin under INV-5. Option **(c)**, accept the break and document it, is closed.
...
**OQ-4 - who pays for the prose correction? ACCEPTED: ordering (ii), founder, 2026-09-23.** The spec required a human here in its own words (requirements.md:1108-1109, "A human must confirm they accept that, because the alternative is 19 re-reads"); the founder accepted the recommendation and its stated cost. Slice 9 is unblocked.
...
**OQ-6 - the three unreachable sidecars. ACCEPTED: do NOT retire, founder, 2026-09-23.** This design inverts the spec's own recommendation, which is why it needed a human: the spec calls retirement "an explicit, human-authorized exception, not a rule" (requirements.md:1130-1131). The founder accepted the inversion, so no historical sign-off is destroyed: SPEC-019's record is seeded, the two stale records are left untouched, and the migration aborts if any of the three ever classifies as a quarantine candidate.
...
**AC-15 needed one clause from the founder. AMENDED 2026-09-25 (batch amendment 4, commit `e6e76052`, PR #2101).** AC-15's text is about "the migration's pre-flight" without qualification, and this migration has two modes. The **rehash mode** discharges AC-15 literally: at least one known record must classify differently under `preStripSpecHash` and `specHash`, or the run dies. The **seed mode** runs pre-strip, when the two bases are byte-identical, so a literal two-bases assertion there would abort and seed nothing. Its pre-flight carries the **polarity-inverted** proof of equal force: `preStripSpecHash(raw) === specHash(raw)` for every corpus file, aborting if not, which proves the frozen copy has not drifted from the basis the records were actually minted under and which inverts into the two-bases proof the day slice 8 lands. Same function, same fixtures, opposite polarity, both executable. The founder accepted the one-clause amendment and its cost, and AC-15 **on the amendment branch** now states the shared obligation with a distinct form for each mode: rehash proves divergence, seed proves agreement plus a non-zero classified population. In *this* tree AC-15 still carries its original single-mode wording. Both proofs are built, which is strictly more than either reading requires.
...
- **Positive clause, half two: the documented provenance behaviour, with a named home - [fix].** `report()` at `packages/minspec/tests/approval-provenance.test.ts:30-32` is generalised to take the script path (default `SCRIPT` at `:20`). The AC-23 test scaffolds the managed set into a temp git repo, mints a sidecar whose `specHash` is the **old-basis** digest computed in TypeScript by `preStripSpecHash`, and runs the **scaffolded** `scripts/approval-provenance.py` against it. The scaffolded copy imports its sibling `hooks/canonical.py` at load (`scripts/approval-provenance.py:49-50`), so the widened scaffolded hasher is the one under test. The assertion is that it reports exactly what FR-11 documents. **Updated 2026-09-25 with OQ-2(ii)'s acceptance:** that is now `PRE-STRIP BASIS - matches under the pre-ownership-strip hash; re-approve to move it forward`, per the FR-11 and AC-23 amendments on the amendment branch - **not** in this tree, where FR-11 still documents `MISMATCH - a real finding`, which is what a reviewer reading this branch will find and should. The amended AC-23 also forbids satisfying its `canonical.py` half by file existence: the scaffolded hasher is **executed** and required to agree with the shipped TypeScript `specHash` while differing from the frozen pre-strip basis on an ownership-bearing fixture. Writing the assertion against the spec rather than against the recommendation is what forced the amendment to be made explicitly rather than absorbed silently.
...
- **OQ-2 (a), an adopter-side migration.** Ships a corpus-rewriting script that rewrites human sign-offs into repos MinSpec cannot test against, and adds a managed template, moving the pinned membership count at `managed-region-enumeration.test.ts:94`.
...
> **Read this first, because the rest of the section is about a different branch.** None of
> the amendments below is in *this* tree. `specs/minspec/SPEC-070-ownership-hash-exit/requirements.md`
> as this branch carries it is the **pre-amendment** text: its FR-11 paragraph still reads
...
> another branch's work as in-tree fact, and a commit or PR reference is not evidence.)*

SPEC-070 was **APPROVED** when this section was written. **All six items below have since been accepted by the founder and applied on the amendment branch**, batched onto `docs/spec-070-amendments` as PR #2101 - items 1-4 in commit `e6e76052`, items 5-6 in `69c32422`, and the review-blocked rework of 5 and 6 - the OQ-2 register resolution, DR-096, and three smaller repairs the panel asked for - in `964e4fb4`. They were batched deliberately, because each separate edit stales the approval and freezes this spec's 14 declared paths until it is re-approved, so six edits would have cost six re-approvals for one outcome. The consequence is the point rather than a side effect: `facts approval SPEC-070` reports `STALE` **when run on the amendment branch**, which is where it was measured; run in this tree it reports `APPROVED`, because none of the staling edits is here. ➡️ **Once that branch lands, the founder's re-approval is a precondition for every slice that writes a declared path.**
...
**5. FR-11's closing paragraph pinned the adopter consequence to a verdict OQ-2(ii) changes.** (Raised by the OQ-2(ii) acceptance, not found independently.)
Evidence: `:625-631` describes adopters getting `MISMATCH - a real finding` on valid old-basis records. Accepting the third verdict makes that false the day slice 8 ships.
Amendment applied: the paragraph now records the `PRE-STRIP BASIS` verdict **and** what that resolution costs, because a decision's text has to carry its own price - a pre-strip hasher inside the adopter-shipped Python, a new managed template, the membership-count pin move, and a fourth artifact under INV-5.

**6. AC-23 was pinned to the same verdict, and was satisfiable by file existence.**
Evidence: `:827-832` asserted the fixture "receives the widened `canonical.py`", which a file-existence check discharges without the hasher ever running - the artifact-existence proxy the evidence rule rejects - and asserted the provenance tool matches FR-11's `MISMATCH`.
Amendment applied: "widened" is now asserted by **executing** the scaffolded hasher and requiring it to agree with the shipped TypeScript `specHash` while differing from the frozen pre-strip basis on an ownership-bearing fixture; the behavioural half runs the **scaffolded** provenance copy against a temporary git repository and asserts the amended `PRE-STRIP BASIS` verdict.
...
**Decided by the founder on 2026-09-23 and 2026-09-25 - no longer pending:**

- **OQ-4, the prose-correction ordering: ordering (ii) accepted**, with the stated cost - the older "ANY edit voids approval" wording survives in the remaining approved files until each is re-approved for its own reasons. Slice 9 is unblocked.
- **OQ-6, the three unreachable sidecars: do NOT retire, accepted.** The design's inversion of the spec's recommendation stands, so no historical `approvedBy`/`approvedAt` is destroyed.
- **OQ-2(ii), the adopter-facing head verdict: build the third verdict, accepted 2026-09-25**, with its stated cost. FR-11 and AC-23 are amended on `docs/spec-070-amendments` so the shipped behaviour and the approved spec agree; they are **not** amended in this tree. The staleness those amendments cause is therefore also not observable here - `facts approval SPEC-070` reports `STALE` **on the amendment branch**, measured there, and this branch's own copy is unaffected by them. ➡️ The founder's re-approval becomes a hard precondition for slice 8 once that branch lands.
- **OQ-9, denying on an `implements_reason` change: record it, do not deny, accepted 2026-09-25.** The design's inversion of the spec's recommendation stands; `implementsReasonAtApproval` lands in slice 5 and the deny arm is deliberately not built.

**Pending the founder (work continues under the stated assumption in every case):**

- **The AC-15 amendment** (section 8 item 4) and the three citation amendments (section 8
  items 1-3) are **no longer pending**: the founder accepted all four and they are applied on
  `docs/spec-070-amendments`, not here. *(This entry said "pending" while section 8 said
  "applied" - the skeptic caught the contradiction and was right; section 8 had been updated
  and this had not.)* What remains pending is downstream of them and is listed above: the
  spec's re-approval.
