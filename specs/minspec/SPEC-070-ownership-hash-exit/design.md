...
SPEC-070 was **APPROVED** when this section was written. **All six items below have since been accepted by the founder and applied**, batched onto `docs/spec-070-amendments` as PR #2101 - items 1-4 in commit `e6e76052`, items 5-6 in `69c32422`, and the review-blocked rework of 5 and 6 - the OQ-2 register resolution, DR-096, and three smaller repairs the panel asked for - in `964e4fb4`. They were batched deliberately, because each separate edit stales the approval and freezes this spec's 14 declared paths until it is re-approved, so six edits would have cost six re-approvals for one outcome. The consequence is the point rather than a side effect: `facts approval SPEC-070` now reports `STALE`, and ➡️ **the founder's re-approval is a precondition for every slice that writes a declared path.**
...
- **OQ-2(ii), the adopter-facing head verdict: build the third verdict, accepted 2026-09-25**, with its stated cost. FR-11 and AC-23 are amended on PR #2101 so the shipped behaviour and the approved spec agree. Consequence: `requirements.md` is `STALE` and ➡️ the founder's re-approval is now a hard precondition for slice 8.
