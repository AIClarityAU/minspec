...
implements: [.github/workflows/approval-materialize.yml, scripts/materialize-approval.ts]
affects: [CODEOWNERS]
...
# affects: CODEOWNERS gains the FR-1 audience→team globs — modifies-not-owns, since the
# file predates this spec and SPEC-047 owns the audience→file map itself.
...
# SPEC-045: GitHub-native approval for all audiences
...
DR-068 (decision 3) resolves this by making the approval *ingress* multi-surface and
...
- **FR-1 (role routing via CODEOWNERS).** A maintained `CODEOWNERS` maps audience file
  ...
  engineering; extensible to future role globs (SPEC-047). Branch protection "require
...
- Sub-file paragraph projection and the bespoke webview (dropped in DR-068).
- The per-audience auto-approve policy (SPEC-046).

## Traceability

DR-068 (decision 3); SPEC-022 (record + canonical hash); DR-056 (human gate); DR-012
(approve gate); SPEC-047 (audience→file map); EPIC-006.
