...
# SPEC-049: Audience-routable DRs — plain-language summaries + dual acceptance
...
DR-068 treats DRs as another audience-routable approvable. A DR's default audience is
...
DR-specific follow-up flagged during the DR-068 discussion, distinct from spec approval
...
- **FR-3 (dual acceptance).** A `business-relevant` DR routes for dual sign-off: architect
  *acceptance* (DR-029 / `MinSpec: Accept ADR`) **and** product/business approval of the
  ...
  so it inherits hash-binding, honest attribution, and the SPEC-045 GitHub ingress; both
  sign-offs are required for `accepted` (RD-2). Non-business DRs keep single architect
  acceptance.
- **FR-4 (jargon advisory on the lead only).** SPEC-048's jargon advisory applies to the DR
...
DR-068 (DR-audience note); DR-029 (ADR acceptance); SPEC-048 (jargon lead); SPEC-022
