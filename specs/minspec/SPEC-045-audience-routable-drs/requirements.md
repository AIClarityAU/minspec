...
- **FR-1 (audience + flag).** A DR carries an audience like other approvables; default =
  ...
  affect the product (pricing, consent/telemetry, offline-first, accessibility, etc.).
...
- **FR-3 (dual acceptance).** A `business-relevant` DR routes for dual sign-off: architect
  *acceptance* (DR-029 / `MinSpec: Accept ADR`) **and** product/business approval of the
  product-impact summary. Non-business DRs keep single architect acceptance.
...
- **OQ-1.** Who decides `business-relevant` — author self-declaration, or a heuristic prompt?
- **OQ-2.** Does the business half of dual acceptance reuse the SPEC-022 approval record atop
  DR-029 architect acceptance, or a DR-native second signature?
