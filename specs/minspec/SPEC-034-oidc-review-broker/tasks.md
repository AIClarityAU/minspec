...
- [x] **1.3 Mint scoped token (`@octokit/auth-app`).** One repo, `review` profile, TTL = GitHub's fixed 1h (AC-3, corrected in 1.4a). *Verify:* response `repositories`/`permissions`/`expires_at` each asserted on a minted response in unit tests with an injected factory (not an integration test — the live path is wired in 1.4), plus refusal tests for a grant wider than the `review` profile.
- [ ] **1.4 ai-review workflow → apply label as bot (e2e).** Workflow requests OIDC (`id-token: write`), calls broker, applies `ai-review:pass` on a fixture PR (AC-7). *Verify:* fixture PR label event shows `sender = minspec-sdd[bot]`.
  - [x] **1.4a Broker request path wired.** `index.ts` runs route → method → **bearer → audience** → OIDC verify → body shape → claim match → mint, in that order, with the single credential-bearing `return` last. The bearer check precedes the `BROKER_AUDIENCE` check deliberately: answering the misconfiguration 500 first would let an unauthenticated caller distinguish a configured broker from an unconfigured one, so nothing is answered before authentication. `broker-scaffold.test.ts` pins this by asserting 401, not 500, for an unconfigured Worker. Impure edges (`jwks`, token factory) injected via `createFetchHandler`, so the whole path is testable with real RS256 crypto and no network. *Verify:* `broker-handler.test.ts` asserts a refusal at each individual step plus a happy-path control, and `wrangler deploy --dry-run` bundles for the Workers runtime with no `nodejs_compat` flag. The bundle size observed at the time was 125.53 KiB / 30.05 KiB gzip - a dev-time observation, not reproducible from the diff, and recorded as such rather than as checked-in evidence. `app-auth.ts`'s synchronous guards are covered directly by `broker-app-auth.test.ts`; its real `@octokit/auth-app` signing path against a live GitHub remains unexercised until 1.4b.
  - [ ] **1.4b Workflow broker path + e2e observation.** BLOCKED, and on two things rather than one. (i) No broker is deployed: the container's wrangler credential was removed, so `wrangler login` against account `23f1dc88…` is a human step (#1869). (ii) **This repo cannot exercise the broker path at all** - it holds `MINSPEC_APP_ID`/`MINSPEC_APP_PRIVATE_KEY`, so `ai-review.yml` always takes the customer-own-app override of slice 4 and the broker branch would never execute in its CI. Verifying AC-7 needs a repo *without* those secrets, i.e. the voip-sms-inbox dogfood repo. Writing the workflow half before either exists would put unexercised code into the gate that reviews every PR in this repo.

  *Task-shape note (2026-09-12): 1.4 bundles a code change and a live-environment observation with different blockers, which is why it is split above rather than left unticked as a single item. 1.4a is complete and verified; 1.4b has not started.*
