...
# SPEC-047: Audience file-separation authoring discipline + validator
...
DR-068 (decision 1) makes audience = whole files and drops sub-file projection. The whole
approval-routing story (SPEC-045) depends on each audience living in its own file, because
...
- **FR-1 (audience→file map, single source of truth).** Define the map as data: default
  ...
  It is **authoritative**: the SPEC-045 `CODEOWNERS` globs are **generated from it** (with a
...
- **RD-2 (single source) — map is authoritative; CODEOWNERS generated from it.** The
  audience→file map is the one source of truth; SPEC-045's `CODEOWNERS` globs are generated
  and drift-validated from it, offline (Tier-0). Prevents routing silently diverging from
  the map.

## Out of scope

- The jargon/readability rule (SPEC-048); per-paragraph audience typing (dropped Phase C).

## Traceability

DR-068 (decision 1); SPEC-013 (section policy + divider); SPEC-038 (warn→error ratchet);
SPEC-045 (CODEOWNERS routing); EPIC-009.
