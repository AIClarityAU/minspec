...
- **FR-6 (idempotent per head).** If an open PR already exists for the branch, adopt it (report its URL) rather than creating a second one. *Rationale: PR-opening must be safe to retry — a partial failure after the push, or `gh pr create` losing a race against a concurrent open, must report the existing PR rather than error or duplicate. ⚠️ **Corrected 2026-08-07 ([#1313](https://github.com/AIClarityAU/minspec/issues/1313)):** this rationale previously read "a re-approval that reuses a branch must not fan out duplicate PRs". Approval branches are **never** reused — `approvalBranchName` ([`approve-push.ts`](../../../packages/minspec/src/lib/approve-push.ts)) appends a millisecond-precision stamp precisely so two approvals cannot derive the same name — so the `gh pr list --head <branch>` probe can only ever match an identical branch, and never fires for a re-approval. FR-6's scope is one head, not one artifact; see R2 for the artifact-level case, which is accepted rather than mitigated.*
...
| R2 | Re-approving the same artifact opens a second PR | **Accepted, not mitigated** ([#1313](https://github.com/AIClarityAU/minspec/issues/1313)). Branches are never reused (millisecond stamp), so FR-6 structurally cannot dedupe here and never claimed to. `approveSpec` has no already-approved short-circuit, so a second approval always writes a fresh record: at an **unchanged** hash the two sidecars differ only in `approvedAt`, both name the same approver and the same approved bytes, and either merge order leaves a valid record — the second PR is a semantic no-op the human closes in one keystroke. At a **changed** hash the two records are genuinely different, and losing the merge race leaves a sidecar whose `specHash` no longer matches the spec, which the approval check rejects loudly — fails closed and visibly, per constitution invariant 2. The active mitigations considered (adopt-by-artifact, supersede-and-close, branch reuse) each add a network probe, a destructive auto-close, or the branch collision the stamp exists to prevent, to save a keystroke on a rare double-approval |
