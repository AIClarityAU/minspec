...
phases:
  ...
  implement: done   # all 3 slices merged: #801 (warn) 2026-07-16, #828 (backfill) + #829 (flip to error) 2026-07-17
---
...
- **FR-3 — Required for T3/T4 past Clarify, which means AUTHORED BEFORE APPROVAL.** The corpus validator MUST **fail** (error, not warn) when a spec is tier T3 or T4 **and** its Clarify phase is done (plan `in-progress` or later) **and** it declares neither a non-empty `implements:` nor the explicit escape (FR-5). The check asserts the declaration **exists** — it is the missing-direction half of the asymmetry, not a check that listed paths resolve.

  > **"Past Clarify" is not a moment an author can wait for (#1489).** Approval is what advances `plan` to `in-progress` (`phasesForApproval`), so the rule is already armed the moment Plan begins — Plan is not a later chance to add the declaration. Nor is Clarify: once the frontmatter is approved, adding `implements:` edits hashed bytes and stales that approval (`canonical.ts` strips only `status`/`phases`). **The declaration must therefore be authored during Specify.** Read "past Clarify" as *when the validator fires*, never as *when an author may add it*.
  >
  > What happens to a spec that arrives without it depends on FR-7's ratchet, and the honest statement of it is narrower than "refused": under `ownershipDeclaration: error` (this repo) `violationsIntroducedByApproval` rejects the approval write; under the shipped default `warn` the approval proceeds and only the spec-gate consequence bites — the spec's own files stay un-armed. Do not restate this as an unconditional refusal in any shipped surface; that claim is true only in a ratcheted repo (#1491 shipped it unconditionally and was blocked for exactly that).
  >
  > Three agent-written specs shipped a deferral comment on the strength of the earlier wording — SPEC-051, SPEC-058 and SPEC-060 (**not** SPEC-059, which never carried one). The inference was wrong in all three. SPEC-058 and SPEC-060 were caught and backfilled before approval; SPEC-051 was approved without it and turned `main` red (#1323), which is the evidence that the prose ambiguity had a real cost rather than a hypothetical one.
