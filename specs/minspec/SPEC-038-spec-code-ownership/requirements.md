...
- **FR-3 — Required for T3/T4 past Clarify, which means AUTHORED BEFORE APPROVAL.** The corpus validator MUST **fail** (error, not warn) when a spec is tier T3 or T4 **and** its Clarify phase is done (plan `in-progress` or later) **and** it declares neither a non-empty `implements:` nor the explicit escape (FR-5). The check asserts the declaration **exists** — it is the missing-direction half of the asymmetry, not a check that listed paths resolve.
  ...
  > **"Past Clarify" is not a moment an author can wait for (#1489).** Approval is what advances `plan` to `in-progress` (`phasesForApproval`), so the trigger condition and the approval keystroke are the *same event*. There is no interval in which a spec is "at Plan" and still editable-before-the-rule-applies. **The declaration must therefore be authored during Specify, and a spec reaching approval without it is refused** — `violationsIntroducedByApproval` rejects the write rather than letting the error land on `main`. Read "past Clarify" as *when the validator fires*, never as *when an author may add it*. Three agent-written specs (SPEC-058/059/060) each deferred the declaration to Plan on the strength of the earlier wording, and all three were un-approvable until it was backfilled.
