...
- [x] **(impl)** `.minspec/config.json`: add `"ownershipDeclaration": "warn"`; `validateOwnership` reads it for the `missing` severity (default `warn` when key absent). *(FR-7 seam)* — allowlist: `.minspec/config.json` — **landed:** PR #801
- [x] **(test, T2 — feature)** `packages/minspec/tests/ownership.test.ts`: AC-1 (missing→fail), AC-2 (`none`+reason→pass), AC-2b (`none` **without** reason→fail), AC-3 (greenfield non-existent path→pass), AC-4 (absolute/escape→`invalid`), AC-6 (T1/T2 exempt), and the **phase predicate** across every `phases.clarify` state (specify/clarify/done) — P2 risk. — allowlist: `packages/minspec/tests/ownership.test.ts` — **landed:** PR #801
- [x] **(test, T2 — integration, AC-5)** drive the real hook: an unapproved T3 spec declaring `implements: ["…/new-thing.ts"]` (non-existent) → `scripts/hooks/spec-gate.py` blocks **creation** of that file, proving the produced signal arms the built gate. — allowlist: `packages/minspec/tests/ownership.test.ts` (spawns the hook; no gate edit) — **landed:** PR #801
- [x] **(verify)** `npm run validate` green on the current corpus (rule at `warn` → existing undeclared specs warn, do not fail); `npm test` green. Confirms Slice 1 is inert-safe to merge. — **landed:** PR #801

## Slice 2 — backfill existing specs (AC-8) · **separate PR**

- [x] Enumerate every T3/T4 spec whose `phases.clarify === 'done'` and that lacks `implements:`; for each, declare `implements:`/`affects:` (or `implements: none` + reason) — **per-spec, reviewed** (P3: a wrong declaration mis-scopes the gate). — allowlist: the enumerated `specs/minspec/*/requirements.md` — **landed:** PR #828
- [x] **(test, AC-8)** with `ownershipDeclaration` forced to `"error"` in the test, the whole corpus validates green. — allowlist: `packages/minspec/tests/ownership.test.ts` — **landed:** PR #828 — satisfied by `npm run validate` against the real corpus at `error`, not a fixture test

## Slice 3 — flip to `error` (FR-7) · **separate PR, after Slice 2 green**

- [x] `.minspec/config.json`: `"ownershipDeclaration": "error"`. One line; the rule now blocks. — allowlist: `.minspec/config.json` — **landed:** PR #829
